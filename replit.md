...
## Backup System (Added 2026-02-16)
- **Route:** `routes/backup.py` with `backup_bp` blueprint
- **Template:** `templates/admin_backup.html`
- **Features:**
  - Full database backup to JSON with type-preserving serialization (datetime, Decimal, UUID, bytes, etc.)
  - Cloud backup to Replit Object Storage (compressed .json.gz)
  - Download backup to local computer
  - FK-safe restore using `session_replication_role = replica` to bypass FK constraints during restore
  - Restore from cloud or uploaded file
  - Automatic sequence reset after restore
  - Admin-only access with sidebar menu item
- **Backup covers all tables** listed in BACKUP_TABLES constant (60+ tables)